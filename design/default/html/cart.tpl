...
<h1>
    {if $cart->has_purchases()}В корзине {$cart->get_total_products()} {$cart->get_total_products()|plural:'товар':'товаров':'товара'}
...
{if $cart->has_purchases()}
    <form method="post" name="cart">
        ...
        <table id="purchases">
            ...
            {foreach $cart->get_purchases() as $purchase}
                <tr>
                    {* Изображение товара *}
                    <td class="image">
                        {$product = $purchase->get_product()}
                        {$variant = $purchase->get_variant()}
                        {$image = $purchase->get_image()}
                        {if $image}
                            <a href="products/{$product->url}">
                                <img
                                    src="{$image->filename|resize:50:50}"
                    ...
                    {* Количество *}
                    <td class="amount">
                        <select name="amounts[{$variant->id}]" onchange="document.cart.submit();">
                            {section name=amounts start=1 loop=$variant->getStock()+1 step=1}
                                <option
                                    value="{$smarty.section.amounts.index}"
                                    {if $purchase->get_amount() == $smarty.section.amounts.index}selected{/if}
                    ...
                    {* Цена *}
                    <td class="price">
                        {($purchase->get_cost())|convert}&nbsp;{$currency->sign}
                    </td>

                    {* Удалить из корзины *}
                    <td class="remove">
                        <a href="cart/remove/{$variant->id}">
                            <img src="design/{$settings->theme}/images/delete.png" title="Удалить из корзины"
            ...
            {if $coupon_request}
                <tr class="coupon">
                    ...
                    <th class="name" colspan="3">Код купона или подарочного ваучера
                        ...
                        <div>
                            <input type="button" name="apply_coupon" value="Применить купон"
                    ...
                    <li>
                        <div class="checkbox">
                            <input type="radio" name="delivery_id" value="{$delivery->id}"
                                   {if $delivery_id==$delivery->id}checked{elseif $delivery@first}checked{/if}
