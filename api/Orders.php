...
class Orders
{
	...
	function get_orders($filter = array())
	{
		...
		if(!empty($filter['keyword']))
		{
			...
			foreach($keywords as $keyword) {
				$keyword_filter .= db()->placehold('AND (o.id = "'.db()->escape(trim($keyword)).'" OR o.name LIKE "%'.db()->escape(trim($keyword)).'%" OR REPLACE(o.phone, "-", "")  LIKE "%'.db()->escape(str_replace('-', '', trim($keyword))).'%" OR o.address LIKE "%'.db()->escape(trim($keyword)).'%" )');
            }
		}
	...
	function count_orders($filter = array())
	{
		...
		if(!empty($filter['keyword']))
		{
			$keywords = explode(' ', $filter['keyword']);
			foreach($keywords as $keyword) {
				$keyword_filter .= db()->placehold('AND (o.name LIKE "%'.db()->escape(trim($keyword)).'%" OR REPLACE(o.phone, "-", "")  LIKE "%'.db()->escape(str_replace('-', '', trim($keyword))).'%" OR o.address LIKE "%'.db()->escape(trim($keyword)).'%" )');
            }
		}
	...
	{
		$query = db()->placehold("UPDATE __labels SET ?% WHERE id in(?@) LIMIT ?", $label, (array)$id, count((array)$id));
		db()->query($query);
		return $id;
	}
	...
	{	
		...
		if($order->closed && !empty($purchase->amount))
		{
			if($old_purchase->variant_id != $purchase->variant_id)
			{
				...
				{
					$query = db()->placehold("UPDATE __variants SET stock=stock+? WHERE id=? AND stock IS NOT NULL LIMIT 1", $old_purchase->amount, $old_purchase->variant_id);
					db()->query($query);
				}
				if(!empty($purchase->variant_id))
				{
					$query = db()->placehold("UPDATE __variants SET stock=stock-? WHERE id=? AND stock IS NOT NULL LIMIT 1", $purchase->amount, $purchase->variant_id);
					db()->query($query);
				}
			}
			elseif(!empty($purchase->variant_id))
			{
				$query = db()->placehold("UPDATE __variants SET stock=stock+(?) WHERE id=? AND stock IS NOT NULL LIMIT 1", $old_purchase->amount - $purchase->amount, $purchase->variant_id);
				db()->query($query);
			}
		}
	...
	public function add_purchase($purchase)
	{
		...
		if($order->closed && !empty($purchase->amount) && !empty($variant->id))
		{
			...
			$query = db()->placehold("UPDATE __variants SET stock=stock-? WHERE id=? AND stock IS NOT NULL LIMIT 1", $stock_diff, $variant->id);
			db()->query($query);
		}
	...
	public function delete_purchase($id)
	{
		...
		{
			$stock_diff = $purchase->amount;
			$query = db()->placehold("UPDATE __variants SET stock=stock+? WHERE id=? AND stock IS NOT NULL LIMIT 1", $stock_diff, $purchase->variant_id);
			db()->query($query);
		}
	...
	public function pay($order_id)
	{
		...
		$query = db()->placehold("UPDATE __orders SET payment_status=1, payment_date=NOW(), modified=NOW() WHERE id=? LIMIT 1", $order->id);
		db()->query($query);
		return $order->id;
	}
	...
	private function update_total_price($order_id)
	{
		...
		$query = db()->placehold("UPDATE __orders o SET o.total_price=IFNULL((SELECT SUM(p.price*p.amount)*(100-o.discount)/100 FROM __purchases p WHERE p.order_id=o.id), 0)+o.delivery_price*(1-o.separate_delivery)-o.coupon_discount, modified=NOW() WHERE o.id=? LIMIT 1", $order->id);
		db()->query($query);
		return $order->id;
	}
