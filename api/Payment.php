...
class Payment
{
    public function get_payment_methods($filter = [])
    {
        ...
        if (!empty($filter['delivery_id'])) {
            $delivery_filter = db()->placehold('AND id in (SELECT payment_method_id FROM __delivery_payment dp WHERE dp.delivery_id=?)',
                intval($filter['delivery_id']));
        }
    ...
    public function update_payment_settings($method_id, $settings)
    {
        ...
        $query = db()->placehold("UPDATE __payment_methods SET settings=? WHERE id in(?@) LIMIT 1", $settings,
            (array) $method_id);
        db()->query($query);
        return $method_id;
    }
