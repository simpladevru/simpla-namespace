...
class Image
{
    ...
    {
        [$source_file, $width, $height, $set_watermark] = $this->get_resize_params($filename);
        ...
        if (class_exists('Imagick') && $this->config->use_imagick) {
            $this->image_constrain_imagick($originals_dir . $original_file, $preview_dir . $resized_file, $width,
                $height, $watermark, $watermark_offet_x, $watermark_offet_y, $watermark_transparency, $sharpen);
        } else {
            $this->image_constrain_gd($originals_dir . $original_file, $preview_dir . $resized_file, $width, $height,
                $watermark, $watermark_offet_x, $watermark_offet_y, $watermark_transparency);
        }
    ...
    public function upload_image($filename, $name)
    {
        ...
        if (in_array(strtolower($ext), $this->allowed_extentions)) {
            ...
            if (move_uploaded_file($filename,
                $this->config->root_dir . $this->config->original_images_dir . $new_name)) {
                return $new_name;
            }
        }
    ...
    private function image_constrain_imagick(
        ...
        // Размеры превью при пропорциональном уменьшении
        [$dst_w, $dst_h] = $this->calc_contrain_size($src_w, $src_h, $max_w, $max_h);
    ...
    /**
     * Вычисляет размеры изображения, до которых нужно его пропорционально уменьшить, чтобы вписать в квадрат $max_w x
     * $max_h
     *
