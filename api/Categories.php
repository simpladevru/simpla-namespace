...
class Categories
{
    ...
    public function get_categories($filter = [])
    {
        ...
        if (!empty($filter['product_id'])) {
            $query = db()->placehold("SELECT category_id FROM __products_categories WHERE product_id in(?@) ORDER BY position",
                (array) $filter['product_id']);
            db()->query($query);
    ...
    {
        $query = db()->placehold("SELECT product_id, category_id, position FROM __products_categories WHERE product_id in(?@) ORDER BY position",
            (array) $product_id);
        db()->query($query);
        return db()->results();
    }
    ...
    {
        $query = db()->placehold("INSERT IGNORE INTO __products_categories SET product_id=?, category_id=?, position=?",
    ...
    {
        $query = db()->placehold("DELETE FROM __products_categories WHERE product_id=? AND category_id=? LIMIT 1",
            intval($product_id), intval($category_id));
        db()->query($query);
    }
        ...
        while (!empty($categories) && !$finish) {
            ...
            foreach ($categories as $k => $category) {
                if (isset($pointers[$category->parent_id])) {
                    ...
                    $pointers[$category->id]->path = array_merge((array) $pointers[$category->parent_id]->path,
                        [$curr]);
        ...
        foreach ($ids as $id) {
            if ($id > 0) {
                ...
                if (isset($pointers[$pointers[$id]->parent_id]->children)) {
                    $pointers[$pointers[$id]->parent_id]->children = array_merge($pointers[$id]->children,
