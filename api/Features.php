...
class Features
{
	function get_features($filter = array())
	{
		...
		if(isset($filter['category_id']))
			$category_id_filter = db()->placehold('AND id in(SELECT feature_id FROM __categories_features AS cf WHERE cf.category_id in(?@))', (array)$filter['category_id']);
	...
	{
		$query = db()->placehold("UPDATE __features SET ?% WHERE id in(?@) LIMIT ?", (array)$feature, (array)$id, count((array)$id));
		db()->query($query);
		return $id;
	}
	...
	{
		$query = db()->placehold("DELETE FROM __options WHERE product_id=? AND feature_id=? LIMIT 1", intval($product_id), intval($feature_id));
		db()->query($query);
	}
	...
	{	 
		if($value != '')
			$query = db()->placehold("REPLACE INTO __options SET value=?, product_id=?, feature_id=?", $value, intval($product_id), intval($feature_id));
		else
			$query = db()->placehold("DELETE FROM __options WHERE feature_id=? AND product_id=?", intval($feature_id), intval($product_id));
	...
	{
		$query = db()->placehold("INSERT IGNORE INTO __categories_features SET feature_id=?, category_id=?", $id, $category_id);
		db()->query($query);
	}
	...
	public function update_feature_categories($id, $categories)
	{
		...
		if(is_array($categories))
		{
			...
			$query = db()->placehold("INSERT INTO __categories_features (feature_id, category_id) VALUES ".implode(', ', $values));
			db()->query($query);
	...
	public function get_options($filter = array())
	{
		...
		if(isset($filter['category_id']))
			$category_id_filter = db()->placehold('INNER JOIN __products_categories pc ON pc.product_id=po.product_id AND pc.category_id in(?@)', (array)$filter['category_id']);

		if(isset($filter['visible']))
			$visible_filter = db()->placehold('INNER JOIN __products p ON p.id=po.product_id AND visible=?', intval($filter['visible']));

		if(isset($filter['brand_id']))
			$brand_id_filter = db()->placehold('AND po.product_id in(SELECT id FROM __products WHERE brand_id in(?@))', (array)$filter['brand_id']);

		if(isset($filter['features']))
			foreach($filter['features'] as $feature=>$value)
			{
				$features_filter .= db()->placehold('AND (po.feature_id=? OR po.product_id in (SELECT product_id FROM __options WHERE feature_id=? AND value=? )) ', $feature, $feature, $value);
			}
