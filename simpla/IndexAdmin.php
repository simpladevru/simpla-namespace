...
class IndexAdmin extends \Root\api\Simpla
{
    private $module;
    ...
    function fetch()
    {
        ...
        $module_class_array = explode('\\', get_class($this->module));
        $module = end($module_class_array);

        // Проверка прав доступа к модулю
        if(isset($this->modules_permissions[$module])
            && $this->managers->access($this->modules_permissions[$module]))
        {
