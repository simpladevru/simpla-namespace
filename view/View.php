...
abstract class View
{
    ...
    public function __construct()
    {
        if (self::$view_instance) {
            ...
            $this->currencies = Simpla()->money->get_currencies(['enabled' => 1]);
            ...
            // Содержимое корзины
            Simpla::$container->design->assign('cart', Simpla::$container->cart);

            // Категории товаров
            Simpla::$container->design->assign('categories', Simpla::$container->categories->get_categories_tree());

            // Страницы
            $pages = Simpla::$container->pages->get_pages(['visible' => 1]);
