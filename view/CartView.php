...
class CartView extends View
{
    ...
    public function __construct()
    {
        ...
        if ($delete_variant_id = intval($this->request->get('delete_variant'))) {
            $cart->delete_purchase($delete_variant_id);
            $cart->save();
        ...
        if (isset($_POST['checkout'])) {
            ...
            if (empty($order->name)) {
                ...
                foreach ($this->request->post('amounts') as $variant_id => $amount) {
                    $this->orders->add_purchase(['order_id'   => $order_id,
                ...
                if (!empty($delivery) && $delivery->free_from > $order->total_price) {
                    $this->orders->update_order($order->id,
                        ['delivery_price' => $delivery->price, 'separate_delivery' => $delivery->separate_payment]);
                }
            ...
            if ($amounts = $this->request->post('amounts')) {
                foreach ($amounts as $variant_id => $amount) {
                    $cart->update_purchase($variant_id, $amount);
                }
...
//                }
    ...
    {
        // Способы доставки
        $deliveries = Simpla::$container->delivery->get_deliveries(['enabled' => 1]);
        $this->design->assign('deliveries', $deliveries);

        // Данные пользователя
        if ($this->user) {
            $last_order = Simpla::$container->orders->get_orders(['user_id' => $this->user->id, 'limit' => 1]);
        ...
        if (Simpla::$container->coupons->count_coupons(['valid' => 1]) > 0) {
